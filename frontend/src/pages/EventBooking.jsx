...
const EventBooking = () => {
  ...
  const fetchDealDetails = async (dealId) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/v1/offers?limit=1000&is_active=true`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          if (foundDeal) {
            ...
            setError(`Deal not found or no longer available. The deal may have expired or the partner may not be approved.`);
          }
        } else {
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      ...
      if (deal.service_type === 'events' && bookingDate) {
        ...
        console.log('📅 Event booking time:', { bookingTime, final: bookingData.booking_time, bookingDate: bookingData.booking_date });
      }
      ...
      if (response.ok && result.success) {
        ...
        if (errorMsg.includes('not found') || errorMsg.includes('expired') || errorMsg.includes('not approved')) {
          setError(`This deal is no longer available for booking. It may have expired or the partner is not approved. Please try selecting a different deal.`);
        } else {
          setError(errorMsg);
        }
      }
  ...
  const calculateTotal = () => {
    if (!deal) return 0;
    const price = parseFloat(deal.discounted_price || deal.original_price || deal.price || 0);
            ...
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <div style={{
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Booking Reference</div>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Deal</div>
                  <div style={{ fontWeight: '600' }}>{deal?.title || 'N/A'}</div>
                </div>
                
                {bookingDate && (
                  <div style={{ marginBottom: '1rem' }}>
                    <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Date & Time</div>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Guests/Tickets</div>
                ...
                <div style={{
                  ...
                  <div style={{ fontWeight: 'bold', color: '#059669', marginBottom: '0.5rem' }}>
                    💳 Payment Instructions
                  </div>
                  <div style={{ fontSize: '0.9rem', color: '#666' }}>
                    Please pay directly at the partner venue when you arrive. Show your booking reference to redeem.
                  </div>
                </div>
              </div>
          ...
          <div className="elizian-auth-modal" style={{ maxWidth: '600px' }}>
            ...
            {deal && (
              <div style={{
                ...
                <h3 style={{ margin: '0 0 0.5rem 0', color: '#333' }}>{deal.title}</h3>
                <p style={{ margin: '0 0 0.5rem 0', color: '#666', fontSize: '0.9rem' }}>
                  {deal.partner_name || deal.location}
                </p>
                <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#059669' }}>
                  {formatPrice(deal.discounted_price || deal.original_price || deal.price)}
                </div>
              </div>
            )}
            ...
            <div style={{
              ...
              <h4 style={{ margin: '0 0 1rem 0', color: '#333' }}>Booking Details</h4>
              
              {bookingDate && (
                <div style={{ marginBottom: '0.75rem' }}>
                  <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Date</div>
                  <div style={{ fontWeight: '600' }}>{formatDate(bookingDate)}</div>
                </div>
              )}
              ...
              {bookingTime && (
                <div style={{ marginBottom: '0.75rem' }}>
                  ...
                  <div style={{ fontWeight: '600' }}>{formatTime(bookingTime)}</div>
                </div>
              )}
              
              <div style={{ marginBottom: '0.75rem' }}>
                <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>
                  {deal?.service_type === 'dining' ? 'Number of Guests' : 'Number of Tickets'}
                </div>
                <div style={{ fontWeight: '600' }}>{numTickets}</div>
              </div>
              ...
              {specialRequests && (
                <div style={{ marginBottom: '0.75rem' }}>
                  <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '0.25rem' }}>Special Requests</div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.2rem', fontWeight: 'bold', color: '#059669' }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '0.9rem', color: '#856404' }}>
                You will pay directly at the partner venue when you arrive. This booking reserves your spot.
              </div>
            </div>
        ...
        <div className="elizian-auth-modal" style={{ maxWidth: '600px' }}>
          ...
          {deal && (
            <div style={{
              ...
              <h3 style={{ margin: '0 0 0.5rem 0', color: '#333' }}>{deal.title || deal.name}</h3>
              <p style={{ margin: '0 0 0.5rem 0', color: '#666', fontSize: '0.9rem' }}>
                {deal.partner_name || deal.location}
              </p>
              {deal.description && (
          ...
          <form className="elizian-auth-form" onSubmit={handleSelectionSubmit}>
              ...
              <>
                <div className="elizian-auth-form-group">
                  <label className="elizian-auth-label" htmlFor="bookingDate">
                    {deal?.service_type === 'events' ? 'Event Date' : 'Booking Date'}
                  </label>
                  <input
                    id="bookingDate"
                    type="date"
                ...
                <div className="elizian-auth-form-group">
                  <label className="elizian-auth-label" htmlFor="bookingTime">
                    {deal?.service_type === 'events' ? 'Event Time' : 'Booking Time'}
                  </label>
                  <input
                    id="bookingTime"
                    type="time"
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span>Price per {deal?.service_type === 'dining' ? 'guest' : 'ticket'}:</span>
                <span>{formatPrice(deal?.discounted_price || deal?.original_price || deal?.price || 0)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span>Quantity:</span>
