...
(function() {
  ...
  function trackView(type, id) {
    ...
    try {
      socket.emit('track_view', { type, id, timestamp: new Date().toISOString() });
    } catch (error) {
  ...
  const realtimeService = {
    ...
    get enabled() { return ENABLE_REALTIME; }
  };
  ...
  console.log(ENABLE_REALTIME ? '🔴 Real-time service loaded (enabled)' : '🟡 Real-time service loaded (disabled)');
