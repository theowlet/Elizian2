...
async function getOfferDiscount(offerId) {
  ...
  const result = await pool.query(
    `SELECT discount_percentage, discount_amount, offer_type, title FROM partner_offers WHERE id = $1`,
    [offerId]
  );
 ...
 * - If offer has discount_percentage > 0: discount = totalBill * (discount_percentage / 100)
...
function calculateRedemptionAmounts(offerId, totalBillAmount, offerRow = null) {
  ...
  const pct = offerRow ? parseFloat(offerRow.discount_percentage) || 0 : 0;
  const fixedDiscount = offerRow ? parseFloat(offerRow.discount_amount) || 0 : 0;

  let discountAmount;
  let effectivePercentage = pct;
  if (pct > 0) {
    discountAmount = Math.round((total * (pct / 100)) * 100) / 100;
 ...
 */
async function calculateRedemptionBreakdown(offerId, totalBillAmount, userId = null) {
  const offer = await getOfferDiscount(offerId);
  const amounts = calculateRedemptionAmounts(offerId, totalBillAmount, offer);
