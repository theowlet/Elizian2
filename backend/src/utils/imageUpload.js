...
function getMimeTypeFromBase64(base64Data) {
  ...
  if (base64Clean.startsWith("R0lGODdh") || base64Clean.startsWith("R0lGODlh"))
    return "image/gif";
...
async function handleImageUpload(
  ...
  log(
    `🔍 handleImageUpload called: subFolder=${subFolder}, prefix=${prefix}, has_base64=${!!image_base64}, filename=${
      image_filename || "none"
    }`
  );
  ...
  try {
    ...
    if (buffer.length > MAX_FILE_SIZE) {
      throw new AppError(
        ...
        }MB limit (received: ${(buffer.length / 1024 / 1024).toFixed(2)}MB)`
      );
    }
    ...
    const safeName =
      (image_filename && image_filename.replace(/[^a-zA-Z0-9-_\.]/g, "")) ||
    ...
    const fileForS3 = {
      buffer: buffer, // The binary data created via Buffer.from()
      originalname: safeName, // The unique name generated in handleImageUpload
      mimetype: mimeType, // The type detected (image/jpeg, etc.)
    };
    // Return relative path for localhost (will be served by Express static middleware).
    // const imageUrl = `/uploads/${subFolder}/${safeName}`;
    const imageUrl = await uploadToS3(fileForS3);
    log(
      `✅ Image uploaded to: ${imageUrl} (${(buffer.length / 1024).toFixed(
        1
      )} KB)`
    );
